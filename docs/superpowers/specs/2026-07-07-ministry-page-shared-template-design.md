...
`kt-ministry-page` 클래스를 공유하는 사실상의 공용 템플릿 구조(Intro → Overview → Journey → Photo Carousel →
...
- [ ] Given 새로 추가되는 모든 텍스트, When `docs/DESIGN.md` 기준으로 점검하면, Then hex 리터럴 직접
      사용 없이 기존 `kt-ministry-*` 클래스 패턴을 재사용하고, 3-column 아이콘-원형 카드 같은 AI Slop
      패턴을 사용하지 않는다.
...
**Programs & Meeting Times** (섹션 래퍼: `.kt-ministry-programs`, 내부는 `.kt-data-table` 재사용)
- 표 컬럼: 모임/프로그램명, 대상, 요일·시간, 장소, 담당자 (`kt-data-table` th/td 그대로 사용)
- 항목 수와 무관하게 5개 페이지 모두 테이블 하나로 통일 → variant 분기 없이 완전히 동일한 마크업 재사용
- 새가족 페이지는 "새가족 4주 과정 + 안내팀 배정" 1~2행만 표시

**Leadership** (섹션 래퍼: `.kt-ministry-leadership`, 선택적 섹션)
- `page-people.html`에서 이미 쓰는 `.kt-people-grid` + `.kt-people-card`(이미지 + `h3` 이름 + `p` 직분)
  그대로 재사용. 카드 개수가 1~3개로 적으므로 `.kt-ministry-leadership .kt-people-grid`에 한해
...
**Testimonials** (섹션 래퍼: `.kt-ministry-testimonials`, 내부는 `.kt-library-quote-grid` 재사용)
...
│  (재사용: design-library의 .kt-data-table)                         │
│  ┌────────────────────────────────────────────────────────────┐  │
...
│  (재사용: page-people.html의 .kt-people-grid + .kt-people-card)    │
│  ┌────────┐  ┌────────┐                                          │
...
│  └────────┘  └────────┘                                          │
├──────────────────────────────────────────────────────────────────┤
│ ⑤ PHOTO CAROUSEL — [ktheme_photo_carousel] 숏코드                  │
│   ◀  [사진] [사진] [사진] [사진]  ▶                                │
├──────────────────────────────────────────────────────────────────┤
│ ⑥ TESTIMONIALS 🆕 — 라벨 · H2 · 설명 한 줄                          │
│  (재사용: design-library의 .kt-library-quote-grid figure+blockquote)│
│ ┌────────────────┐ ┌────────────────┐ ┌────────────────┐         │
...
- `style.css`: 신규 클래스 최소화. 다음 세 가지만 추가한다.
  1. `.kt-ministry-leadership .kt-people-grid` 컬럼 수 override (`repeat(auto-fit, minmax(200px,1fr))`)
  2. `.kt-ministry-testimonials .kt-library-quote-grid` 풀 너비 3열 override + 반응형
  3. `.kt-library-quote-grid figure cite` 작성자 표기 스타일 (다크 variant 포함)
- 5개 템플릿 파일:
  - 기존 `.kt-ministry-journey` 섹션 제거
  - `.kt-ministry-programs`(`.kt-data-table` 재사용), `.kt-ministry-leadership`(`.kt-people-grid`/
    `.kt-people-card` 재사용), `.kt-ministry-testimonials`(`.kt-library-quote-grid` 재사용) 섹션 추가
