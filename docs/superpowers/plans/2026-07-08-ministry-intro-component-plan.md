...
## 대상 범위
공동체 관련 6개 페이지의 최상단 Intro 섹션(`.modu-ministry-intro`):
...
- 6개 페이지 모두 이미 **동일한 HTML 구조**를 쓰고 있음: `.modu-ministry-intro` 그리드
  (콘텐츠 컬럼 `.modu-ministry-intro__content` + 미디어 컬럼 `.modu-ministry-intro__media`),
  콘텐츠 컬럼은 eyebrow(`modu-card-label`) + `h2` + `p` + 버튼 2개(`modu-buttons`), 미디어 컬럼은
  `img` + 우하단 오버레이 노트(`modu-ministry-intro__note`). 구조적 차이는 없고 일부 페이지(next-generation,
...
- 현재 CSS(`style.css` `.modu-ministry-intro__media`)는 `min-height: 390px`만 지정하고 있고, 미디어
...
- **CSS**: `.modu-ministry-intro__media`에 `aspect-ratio: 612 / 390`을 지정해, 컬럼 너비가 화면 크기에
...
### T-1: CSS 표준화 — `.modu-ministry-intro__media` 612:390 비율 고정
- **Files**: `wp-content/themes/modu-theme/style.css`
- **Change**:
  1. `.modu-ministry-intro__media`에 `aspect-ratio: 612 / 390;` 추가 (기존 `min-height: 390px`는
     구형 브라우저 폴백 겸용으로 유지 여부 검토 후 정리)
  2. `.modu-ministry-intro__media img`는 기존 `width:100%; height:100%; object-fit:cover;` 유지
  3. 640px 반응형 블록의 `.modu-ministry-intro__media, .modu-ministry-intro__media img { min-height: 320px; }`
...
- **Files**: `wp-content/themes/modu-theme/templates/page-design-library.html` (`#ministry` 블록,
  "Intro" 행)
...
- **Change**: 각 페이지 `.modu-ministry-intro__media img` 태그에 `width="612" height="390"` 속성만
  추가. 기존 카피, 링크, 이미지 파일, 노트 텍스트는 변경하지 않음(최소 diff 원칙)
- **Verify**: `grep -n "modu-ministry-intro__media" -A1` 로 6개 파일 모두 `width="612" height="390"`
  포함 확인
