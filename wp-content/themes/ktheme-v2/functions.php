...
function ktheme_v2_render_utility_menu(): string {
	...
	$html = '<nav class="kt-utility-nav" aria-label="' . esc_attr__( '?ÅÎã® Î©îÎâ¥', 'ktheme-v2' ) . '">';
	foreach ( $items as $item ) {
...
function ktheme_v2_render_primary_menu(): string {
	...
	$total = count( $items );
	$html  = '<nav class="kt-nav" aria-label="' . esc_attr__( 'Ï£ºÏöî Î©îÎâ¥', 'ktheme-v2' ) . '"><ul class="kt-nav__list">';
...
function ktheme_v2_render_mega_menu(): string {
	...
	$html .= '<div class="kt-mega-menu__head"><strong>' . esc_html__( '?ÑÏ≤¥ Î©îÎâ¥', 'ktheme-v2' ) . '</strong><span>' . esc_html__( 'Í∞Ä?âÍµê?åÏùò ?àÎ∞∞, Í≥µÎèôÏ≤? ?ëÏú°Í≥??âÏ†ï ?àÎÇ¥Î•??úÍ≥≥?êÏÑú ?ïÏù∏?òÏÑ∏??', 'ktheme-v2' ) . '</span></div>';
...
function ktheme_v2_render_header_shortcode(): string {
	...
	<div class="kt-topbar">
		<div class="kt-container kt-topbar__inner">
			<div><span class="kt-dot"></span><?php echo esc_html__( 'Ï£ºÏùº?àÎ∞∞ ¬∑ ?§Ï†Ñ 11:00 Î≥∏Îãπ', 'ktheme-v2' ); ?></div>
	...
	<div class="kt-header">
		<div class="kt-container kt-header__inner">
			<a href="<?php echo esc_url( home_url( '/' ) ); ?>" class="kt-brand kt-brand--mark-only" aria-label="<?php echo esc_attr__( 'Í∞Ä?âÍµê????', 'ktheme-v2' ); ?>">
				<img src="<?php echo esc_url( get_theme_file_uri( 'assets/images/logos/hanbit-book-light-color.svg' ) ); ?>" alt="<?php echo esc_attr__( 'Í∞Ä?âÍµê??', 'ktheme-v2' ); ?>" />
			</a>
			...
			<div class="kt-header__actions">
				<button class="kt-icon-button" type="button" aria-label="<?php echo esc_attr__( 'Í≤Ä???¥Í∏∞', 'ktheme-v2' ); ?>" aria-controls="kt-header-search" aria-expanded="false" data-kt-search-toggle>
				...
				<a class="kt-header-register" href="<?php echo esc_url( home_url( '/newcomers/' ) ); ?>"><?php echo esc_html__( '?àÍ?Ï°??±Î°ù', 'ktheme-v2' ); ?></a>
				<button class="kt-mega-toggle" type="button" aria-label="<?php echo esc_attr__( '?ÑÏ≤¥ Î©îÎâ¥ ?¥Í∏∞', 'ktheme-v2' ); ?>" aria-controls="kt-mega-menu" aria-expanded="false" data-kt-mega-toggle>
	...
	<div class="kt-footer">
		<div class="kt-container kt-footer__grid">
			<div class="kt-footer__brand">
				<a href="<?php echo esc_url( home_url( '/' ) ); ?>" class="kt-brand kt-brand--mark-only" aria-label="<?php echo esc_attr__( 'Í∞Ä?âÍµê????', 'ktheme-v2' ); ?>">
					<img src="<?php echo esc_url( get_theme_file_uri( 'assets/images/logos/hanbit-book-light-white.svg' ) ); ?>" alt="<?php echo esc_attr__( 'Í∞Ä?âÍµê??', 'ktheme-v2' ); ?>" />
				</a>
				<p><?php echo esc_html__( '?§Ïùå ?∏Î??Ä ?®Íªò ?àÎ∞∞?òÍ≥†, ÏßÄ??≥º ?ºÏÉÅ???¨Í∏∞??ÍµêÌöå?ÖÎãà??', 'ktheme-v2' ); ?></p>
			</div>

			<nav class="kt-footer-menu-grid" aria-label="<?php echo esc_attr__( '?∏ÌÑ∞ Ï£ºÏöî Î©îÎâ¥', 'ktheme-v2' ); ?>">
				<?php
				if ( $has_footer_menus ) {
		...
		<div class="kt-footer__bottom">
			<div class="kt-container kt-footer__bottom-inner">
				<p><?php echo esc_html__( '08512 ?úÏö∏?πÎ≥Ñ??Í∏àÏ≤úÍµ?Í∞Ä?∞ÎîîÏßÄ??Î°?168', 'ktheme-v2' ); ?></p>
...
function ktheme_v2_render_page_hero_visual( string $style, array $settings ): string {
	...
	return '<div class="kt-page-hero__visual" aria-hidden="true">' .
		...
		'<div class="kt-page-hero__side-card"><strong>' . esc_html__( 'Ï£ºÏùº ?àÎÇ¥', 'ktheme-v2' ) . '</strong><span>' . esc_html__( '1Î∂Ä 09:00 ¬∑ 2Î∂Ä 11:00', 'ktheme-v2' ) . '</span></div>' .
		'</div>';
}
...
function ktheme_v2_render_page_hero_shortcode(): string {
	...
	return '<section class="' . esc_attr( implode( ' ', $classes ) ) . '" style="' . esc_attr( $style_attr ) . '">' .
		$media_html .
		'<nav class="kt-breadcrumb" aria-label="' . esc_attr__( '?ÑÏû¨ ?ÑÏπò', 'ktheme-v2' ) . '">' . $breadcrumb . '</nav>' .
...
function ktheme_v2_render_photo_carousel_shortcode( $atts = array() ): string {
	...
	foreach ( array_values( $items ) as $index => $item ) {
		...
		$dots .= '<button class="kt-photo-carousel__dot" type="button" aria-label="' . esc_attr( sprintf( '%dÎ≤àÏß∏ ?¨ÏßÑ Î≥¥Í∏∞', $index + 1 ) ) . '" aria-controls="' . esc_attr( $slide_id ) . '" data-kt-carousel-dot="' . esc_attr( (string) $index ) . '"><span></span></button>';
	}
	...
	return '<section class="' . esc_attr( implode( ' ', $classes ) ) . '" aria-labelledby="' . esc_attr( $section_id . '-title' ) . '" data-kt-photo-carousel>' .
		'<div class="kt-photo-carousel__head">' .
			...
			'<div class="kt-photo-carousel__controls">' .
				'<button class="kt-photo-carousel__button" type="button" aria-label="' . esc_attr__( '?¥Ï†Ñ ?¨ÏßÑ', 'ktheme-v2' ) . '" data-kt-carousel-prev><svg class="kt-icon kt-icon--sm" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.3" aria-hidden="true"><path d="M15 18 9 12l6-6"/></svg></button>' .
				'<button class="kt-photo-carousel__button" type="button" aria-label="' . esc_attr__( '?§Ïùå ?¨ÏßÑ', 'ktheme-v2' ) . '" data-kt-carousel-next><svg class="kt-icon kt-icon--sm" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.3" aria-hidden="true"><path d="m9 18 6-6-6-6"/></svg></button>' .
			'</div>' .
		'</div>' .
		...
		'<div class="kt-photo-carousel__footer">' .
			'<div class="kt-photo-carousel__dots" aria-label="' . esc_attr__( '?¨ÏßÑ ?†ÌÉù', 'ktheme-v2' ) . '">' . $dots . '</div>' .
...
function ktheme_v2_customize_register_page_hero( WP_Customize_Manager $wp_customize ): void {
	...
	$wp_customize->add_control(
		'ktheme_v2_page_hero_enabled',
		array(
			'label'   => __( 'Í≥µÏö© ?àÏñ¥Î°??úÏãú', 'ktheme-v2' ),
	...
	$wp_customize->add_control(
		'ktheme_v2_page_hero_image_url',
		array(
			'label'       => __( 'Î∞∞Í≤Ω/?∞Ï∏° ?¥Î?ÏßÄ URL', 'ktheme-v2' ),
			'description' => __( 'Image, Ken Burns, Split ?§Ì??ºÏóê ?¨Ïö©?©Îãà??', 'ktheme-v2' ),
			'section'     => 'ktheme_v2_page_hero',
			'type'        => 'url',
		)
	);
	...
	$wp_customize->add_control(
		'ktheme_v2_page_hero_youtube_url',
		array(
			'label'       => __( '?†ÌäúÎ∏?Î∞∞Í≤Ω ?ÅÏÉÅ URL', 'ktheme-v2' ),
			'description' => __( 'Video Background ?§Ì??ºÏóê ?¨Ïö©?©Îãà??', 'ktheme-v2' ),
			'section'     => 'ktheme_v2_page_hero',
			'type'        => 'url',
		)
	);
	...
	foreach (
		array(
			'ktheme_v2_page_hero_accent_color'  => array( __( 'Í∞ïÏ°∞ ?âÏÉÅ', 'ktheme-v2' ), '#3a64f5' ),
			'ktheme_v2_page_hero_text_color'    => array( __( '?çÏä§???âÏÉÅ', 'ktheme-v2' ), '' ),
			'ktheme_v2_page_hero_overlay_color' => array( __( '?§Î≤Ñ?àÏù¥ ?âÏÉÅ', 'ktheme-v2' ), '#0e1320' ),
