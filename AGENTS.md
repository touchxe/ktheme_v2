...
모든 작업 단계와 결과는 비기술 사용자도 바로 이해할 수 있는 쉬운 한국어로 설명한다.

각 단계의 시작·중간·완료 보고에는 다음 세 항목을 순서대로 포함한다.

1. **무엇을 하는가** — 변경 대상과 작업 범위를 한 문장으로 설명한다.
2. **왜 필요한가** — 사용자·운영·판매 관점의 이유를 한 문장으로 설명한다.
3. **완료되면 무엇이 달라지는가** — 눈에 보이는 결과 또는 다음 단계에 주는 이점을 한 문장으로 설명한다.

검증 결과는 명령어 이름만 나열하지 말고, `확인한 내용 → 결과 → 남은 제한사항` 순서로 풀어쓴다. 약어와 내부 코드명은 꼭 필요할 때만 쓰고, 처음에는 쉬운 표현을 함께 제공한다.
